...
const projectCard = (item) => `
  <button class="archive-card project-card" type="button" data-mode="project" data-id="${item.id}" data-category="${item.categoryKey}">
    <span class="thumb"><img src="${imageFor(item)}" alt="${item.title}" loading="lazy" decoding="async"></span>
    <span class="meta">
...
const imageCard = (item) => `
  <button class="image-tile" type="button" data-mode="direct-image" data-id="${item.id}" data-category="${item.categoryKey}">
    <img src="${item.src}" alt="${item.category}" loading="lazy" decoding="async">
  </button>
`;
...
const openViewer = (items, index, title) => {
  activeItems = items || [];
  activeIndex = index || 0;
  activeTitle = title || "";
...
grid.addEventListener("click", (event) => {
  ...
  if (card.dataset.mode === "project") {
    ...
    if (project) openViewer(project.images, 0, project.title);
    return;
  }

  if (directKeys.has(card.dataset.category)) {
    const index = Number(card.dataset.id.split("-").pop());
    openViewer(category.items, index, category.title);
  }
});
